...
def check_horse_winner(atu):
    alist=[]
    astr=""
    for i in range (0,len(atu)):
        if "E" not in atu[i]:
            alist.append(i)
    ...
    if len(alist)==len(atu):
        return "Everyone: keep playing!"
    elif 1<len(alist)<len(atu):
        return "Players "+astr+ ": keep playing!"
    elif len(alist)==1:
        return "Player "+str(alist[0])+" wins!"
